...
cat <<EOF
Additional CycloneOS functions:
- cout:            Changes directory to out.
- mmp:             Builds all of the modules in the current directory and pushes them to the device.
- mmap:            Builds all of the modules in the current directory and its dependencies, then pushes the package to the device.
- mmmp:            Builds all of the modules in the supplied directories and pushes them to the device.
- cyclonegerrit:   A Git wrapper that fetches/pushes patch from/to CycloneOS Gerrit Review.
- cyclonerebase:   Rebase a Gerrit change and push it again.
- cycloneremote:   Add git remote for CycloneOS Gerrit Review.
- aospremote:      Add git remote for matching AOSP repository.
- cafremote:       Add git remote for matching CodeAurora repository.
- githubremote:    Add git remote for CycloneOS Github.
...
{
    if [ "$OUT" ] ; then
        ZIPPATH=`ls -tr "$OUT"/cyclone-*.zip | tail -1`
        if [ ! -f $ZIPPATH ] ; then
            echo "Nothing to eat"
            return 1
        fi
...
function cycloneremote()
{
    ...
    git remote rm cyclone 2> /dev/null
    ...
    local CYCLONE_USER=$(git config --get review.review.cycloneos.org.username)
    if [ -z "$CYCLONE_USER" ]
    then
        git remote add cyclone ssh://review.CycloneOS.org:29418/$PFX$PROJECT
    else
        git remote add cyclone ssh://$cyclone_USER@review.CycloneOS.org:29418/$PFX$PROJECT
    fi
    echo "Remote 'cyclone' created"
}
...
function githubremote()
{
    ...
    git remote add github https://github.com/CycloneOS/$PROJECT
...
function makerecipe() {
    ...
    then
        pwd
        cycloneremote
        git push cyclone HEAD:refs/heads/'$1'
    fi
    '
}
...
function cyclonegerrit() {
    ...
    local user=`git config --get review.review.CycloneOS.org.username`
...
function cyclonerebase() {
    ...
    if [ -z $repo ] || [ -z $refs ]; then
        echo "CycloneOS Gerrit Rebase Usage: "
    ...
    git fetch "http://review.CycloneOS.org/p/$repo" "refs/changes/$refs" && git cherry-pick FETCH_HEAD
    if [ "$?" != "0" ]; then
